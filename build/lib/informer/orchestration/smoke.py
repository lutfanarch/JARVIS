...
This module is deliberately free of Click imports and does not
import from :mod:`informer.cli` to avoid circular import issues.
"""
...
# Default timeframes used for health checks and smoke test
...
def _healthcheck_pass(engine, run_id: str) -> bool:
    ...
    """
    try:
        symbols: List[str] = list(CANONICAL_WHITELIST)
        ...
        return getattr(report, "status", None) != "NOT_READY"
    except Exception:
        return False
...
def run_smoke_test(
    *,
    ...
    # schema state.  If deletion fails we proceed anyway; upgrade_head
    ...
    try:
        # Step 1: Database initialization via Alembic migrations
