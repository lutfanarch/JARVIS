...
@click.option(
    ...
    default=0.0,
    show_default=True,
    help="Slippage in USD per share per side (overrides slippage-bps; default: 0.0).",
)
@click.option(
    "--daily-pause-limit-usd",
    type=float,
    default=None,
    help="Daily pause limit in USD (absolute loss).",
)
@click.option(
    "--safety-pre-pause-enabled",
    is_flag=True,
    default=False,
    help="Enable safety pre-pause guardrail.",
)
@click.option(
    "--safety-pre-pause-limit-usd",
    type=float,
    default=450.0,
    show_default=True,
    help="Safety pre-pause limit in USD (default: 450.0).",
)
def backtest(
    start: str,
    end: str,
    ...
    daily_pause_limit_usd: Optional[float],
    safety_pre_pause_enabled: bool,
    safety_pre_pause_limit_usd: float,
) -> None:
    ...
    # Determine profile overrides
    profile_name = os.getenv("PROP_PROFILE")
    if profile_name == "trade_the_pool_25k_beginner":
        if daily_pause_limit_usd is not None and daily_pause_limit_usd != 500.0:
            click.echo(
                f"[warning] Overriding user daily_pause_limit_usd {daily_pause_limit_usd} with strict profile limit 500.0",
                err=True,
            )
        daily_pause_limit_usd = 500.0
    ...
    cfg = BacktestConfig(
        ...
        daily_pause_limit_usd=daily_pause_limit_usd,
        safety_pre_pause_enabled=safety_pre_pause_enabled,
        safety_pre_pause_limit_usd=safety_pre_pause_limit_usd,
    )
    # Determine output directory
