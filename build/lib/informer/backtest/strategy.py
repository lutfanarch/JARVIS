...
class BacktestConfig:
    ...
    daily_pause_limit_usd : float, optional
        Daily pause limit in USD (absolute loss).  If the daily PnL
        is less than or equal to the negation of this value, trading
        is blocked.
    safety_pre_pause_limit_usd : float
        Safety pre‑pause limit in USD.  A secondary guardrail that
        blocks trading when daily PnL hits this level (if enabled).
    safety_pre_pause_enabled : bool
        Whether to enable the safety pre‑pause guardrail.
    ``extra_params`` : dict
    ...
    daily_pause_limit_usd: Optional[float] = None
    safety_pre_pause_limit_usd: float = 450.0
    safety_pre_pause_enabled: bool = False
