...
def write_summary_json(
    ...
    if cost_model is not None:
        ...
        cfg_dict["cost_model"] = {
            ...
            "slippage_per_share": cost_model.slippage_per_share,
        }

    # Construct config_effective block
    config_effective = {
        "ttp": {
            "costs_enabled": True,  # Always true in backtest if CostModel is used
            "commission_per_share": cost_model.commission_per_share if cost_model else 0.0,
            "slippage_per_share": cost_model.slippage_per_share if cost_model else 0.0,
            "cost_apply_on": "round_trip",
            "daily_pause_limit_usd": config.daily_pause_limit_usd,
            "safety_pre_pause_enabled": config.safety_pre_pause_enabled,
            "safety_pre_pause_limit_usd": config.safety_pre_pause_limit_usd,
        }
    }
